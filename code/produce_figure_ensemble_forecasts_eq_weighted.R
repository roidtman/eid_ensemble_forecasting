...
# Equally-weighted forecasts on the department level 
#=============================================================================#
...
#=============================================================================#
# create equal ensemble weight output or load it 
#=============================================================================#


if(!file.exists('../output/main_text_figures/eq_wts_ensemble_forecast.RData')){
  ...
  ensemble_wts_through_time = prop.table(ensemble_wts_through_time, 2)
  ...
  for(tt in 1:length(when_to_assimilate)){
    ...
    for(rr in 1:length(models_with_weight)){
      ...
      if(rr == 1){
        ...
        I_F[[tt]][, (sum(num_samps_per_model[models_with_weight[1:(rr - 1)]]) + 1) : 
                    sum(num_samps_per_model[models_with_weight[1:rr]]), ] = 
          I_F_over_time_rho_sample[[tt]][, samps_tmp, ]
        
        I_A[, (sum(num_samps_per_model[models_with_weight[1:(rr - 1)]]) + 1) : 
              sum(num_samps_per_model[models_with_weight[1:rr]]), 
            wks_assimilated_at_when_to_assimilate[tt,]] = 
          I_A_over_time_rho_sample[, samps_tmp, wks_assimilated_at_when_to_assimilate[tt,]]
  ...
  # sampling down I_F and I_A
  s = sample(1:20000, 2000)
  I_A_over_time_rho_sample = I_A[,s,]
  # test = I_A[,s,]
  
  I_F_over_time_rho_sample = list()
  length(I_F_over_time_rho_sample) = length(when_to_assimilate)
  for(tt in when_to_assimilate){
    I_F_over_time_rho_sample[[which(tt == when_to_assimilate)]] = I_F[[which(tt == when_to_assimilate)]][,s,]
  }
  
  save(I_A_CI_rho, I_A_CI_rho_large, I_F_CI_rho, I_F_CI_rho_large,
       I_A_over_time_rho_sample, I_F_over_time_rho_sample,
       file = '../output/main_text_figures/eq_wts_ensemble_forecast.RData')
...
for(tt in times_to_plot){
  f_out = paste0('../output/main_text_figures/supplement/ensemble_forecasts_', 
