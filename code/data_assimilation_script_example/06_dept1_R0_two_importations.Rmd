...
```{r}
file_path = '20200330_no_tm_ini_6'
set.seed(61)
dd_curr = 1
...
particles$tm_ini2 = base::sample(((week_first_case-1) - 35):max(df_dyn$week), np, replace = T)
```
...
for(pp in 1:np){
  ...
  for(tt_sim in particles$tm_ini[pp]:(week_first_case - 1)){
    ...
    if(tt_sim == particles$tm_ini2[pp]){
      df_list[[pp]]$inf_new[which(df_list[[pp]]$week == particles$tm_ini2[pp])] = df_list[[pp]]$inf_new[which(df_list[[pp]]$week == particles$tm_ini2[pp])] + sample(1:5, 1)
    }
