...
## Here, we are using the spatial model with R0 values informed by Perkins et al. 2016 (Nature Microbiology) and are allowing for 2 initial importations (potentially informed by genetic data)
```
...
```{r}
file_path = '20200401_no_tm_ini_14'
set.seed(14)
```
...
load('/dbfs/FileStore/tables/processed_radiation_model.RData')
connectivity = connectivity_radiation_normalized
rm(connectivity_radiation_normalized)
