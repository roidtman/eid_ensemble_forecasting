...
# Comparing forecasts from dynamic and static R models
#=============================================================================#
...
f = paste0('../output/ensemble/comparing_forecasts_across_R.pdf')
pdf(file = f, height = 4, width = 7.5)
...
cols_to_plot_static = c('slategray1', 'slategray3', 'slategray4')
cols_to_plot_dynamic = c('indianred1', 'indianred3', 'indianred4')


par(mar = c(0.5,3.5,0,1), oma = c(3.5,3,3.4,1))
    ...
    for(tt in 1:length(when_to_assimilate)){
      ...
      if(tt %in% times_to_plot){
        ...
        if(tt == 1){
          ...
          if(rr == 1){
            mtext(side = 3, text = 'Static R', line = 1, col = 'slategray4', cex = 0.8)
          }else{
            mtext(side = 3, text = 'Dynamic R', line = 1, col = 'indianred4', cex = 0.8)
          }
        }
